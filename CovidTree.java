...
public class CovidTree {
    ...
    public CovidTree insert(Human p, Human c) throws IllegalArgumentException {
        ...
        if (node(p) == null) { throw new IllegalArgumentException(); }

        if (p == null || c == null) {
            throw new IllegalArgumentException("arguments p and c cannot be null");
        }
        if (node(c) != null) {
            throw new IllegalArgumentException("c is already in this covidtree");
        }

        var node= new CovidTree(c);
        for (CovidTree bruh : children) {
            if (bruh == node(p) && node(p) != null) {
                node(p).children.add(node);
    ...
    public int size() {
        ...
        if (children.size() == 0 && human != null) { return 1; }
    ...
    public boolean contains(Human p) {
        ...
        boolean bruh= false;
        for (CovidTree dt : children) {
            // if (dt.human().equals(p)) { }
            bruh= dt.contains(p);

        }
        return bruh; // bruh might not be necessary
    }
    ...
    public int widthAtDepth(int d) throws IllegalArgumentException {
        ...
        if (d < 0) { throw new IllegalArgumentException("has to be a positive integer"); }
        if (d == 0) { return 1; }

        // ..... If d > 0, the answer is: sum of widths of the children at depth d-1.
        int sum= 0;
        for (CovidTree child : children) {
            sum= sum + child.widthAtDepth(d - 1);
        }
        return sum;
    }
    ...
    public List<Human> CovidRouteTo(Human c) {
        ...
        // search, but if route doesn't exist then it returns null

        // prepend for linked list: addFirst(E e)
        // Base Case

        if (human.equals(c)) {
            LinkedList<Human> bro= new LinkedList<Human>();
            bro.addFirst(c);
            return bro;
        }

        // recursive case: in English first:

        for (CovidTree ch : children) {
            List<Human> part= ch.CovidRouteTo(c);

            if (part != null) { // might be ch != null
                part.add(0, c);
                return part; // might have to be outside of loop
    ...
    public Human commonAncestor(Human child1, Human child2) {
        ...
        Object[] l1= node(human).CovidRouteTo(child1).toArray();

        Object[] l2= node(human).CovidRouteTo(child2).toArray();

        // no recursion - for loop

        for (int i= 0; i < l1.length; i++ ) {
            if (l2[i] == l1[i]) { return (Human) l1[i]; }
        }
    ...
    public boolean equals(Object ob) {
        ...
        if (human != (Human) ob) { return false; }
        if (childrenSize() != ((CovidTree) ob).childrenSize()) { return false; }

        boolean bruh= false;
        for (CovidTree ch : children) {
            if (isInSet(ch, ((CovidTree) ob).children) || isInSet((CovidTree) ob, children)) {
                bruh= true;
            }
        }
        return bruh;
    ...
    public static boolean isInSet(CovidTree t, Set<CovidTree> s) {
        ...
        // might have to use for each loop to process the set of covid trees
        // for every covid tree in covid tree set s, check to see if ch == t, and if it is then
        // boolean = true
        boolean bruh= false;
        for (CovidTree ch : s) {
            if (ch.equals(t)) {
